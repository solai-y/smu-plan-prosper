...
function getEligibility(
  ...
  if (prereqs.length === 0 || prereqs.every((p) => completedCodes.has(p))) return "eligible";
  return "ineligible";
}
          ...
          {module.prerequisites.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {module.prerequisites.map((p) => (
                <Link key={p} to={`/module/${p}`}>
                  <Badge
                    variant="outline"
                    className="font-mono cursor-pointer hover:bg-accent transition-colors"
                  >
                    {p}
                    {completedCodes.has(p) && <CheckCircle className="ml-1 h-3 w-3 text-success" />}
                  </Badge>
                </Link>
              ))}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-2">
              {slots.map((slot) => (
                <div key={slot.id} className="flex items-center gap-3 rounded-md border p-3 text-sm">
